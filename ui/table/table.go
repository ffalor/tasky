// Package table implements the main table view for tasks
package table
...
func New(ctx context.ProgramContext) Model {
	t := table.New(
		...
		table.WithHeight(ctx.ScreenHeight),
	)
...
func (m *Model) SetTableHeight(height int) {
	f, frameH := m.s.Header.GetFrameSize()
	m.table.SetHeight(height - frameH - f)
}
